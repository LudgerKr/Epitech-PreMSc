...
void Vm::load(instruction_t instruction)
{
    ...
    if (reg_address < 0 || reg_address > 15) {
      throw (Vm::VmError("Invalid registry address"));
    }
...
void Vm::store(instruction_t instruction)
{
    ...
    if (this->_stack.empty()) {
      throw (Vm::VmError("Can't store value when stack empty"));
    }
    reg_address = std::stoi(instruction.value);
    if (reg_address < 0 || reg_address > 15) {
      throw (Vm::VmError("Invalid registry adress"));
    }
